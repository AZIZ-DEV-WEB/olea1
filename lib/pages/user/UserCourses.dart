...
class MesCoursPage extends StatefulWidget {
  const MesCoursPage({Key? key}) : super(key: key);

  @override
  _MesCoursPageState createState() => _MesCoursPageState();
}
...
class _MesCoursPageState extends State<MesCoursPage> {
  ...
  // Function to get Firebase formation IDs where the current user is a participant
  Future<List<String>> getParticipatingFormationIds() async {
    ...
    try {
      ...
      for (var doc in formationSnapshot.docs) {
        ...
        if (participants != null) {
          bool isParticipant = participants.any((p) => (p as Map<String, dynamic>)['uid'] == _currentUser!.uid);
          if (isParticipant) {
      ...
      print('Error getting participating formation IDs: $e');
    }
    return formationIds;
  }
  ...
  Widget getAndDisplayUserCourses(String? currentSelectedFormationId) {
    ...
    return FutureBuilder<List<String>>(
      future: getParticipatingFormationIds(),
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          print("You are not invited to any formations with courses.");
          return const Center(child: Text('Vous n\'êtes invité à aucune formation avec des cours.'));
        }

        final List<String> userFormationIds = snapshot.data!;
        print('User is participating in formations: $userFormationIds');

        // Si aucune formation n'est sélectionnée, ou si la formation sélectionnée n'est pas dans celles de l'utilisateur
        if (currentSelectedFormationId == null || !userFormationIds.contains(currentSelectedFormationId)) {
        ...
        return StreamBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, supabaseSnapshot) {
            ...
            if (!supabaseSnapshot.hasData || supabaseSnapshot.data!.isEmpty) {
              ...
              return const Center(child: Text('Aucun cours trouvé pour vos formations.'));
            }
                  ...
                  child: ListTile(
                    ...
                    trailing: documentUrl != null
                      ...
                      onPressed: () => _downloadFile(documentUrl, fileName),
                        ...
                        : null, // N'affiche pas le bouton si pas d'URL de document
                  ),
                );
              },
            );
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                for (var doc in snapshot.data!.docs) {
                  ...
                  if (participants != null && _currentUser != null) {
                    isParticipant = participants.any((p) => (p as Map<String, dynamic>)['uid'] == _currentUser!.uid);
                  }
                ...
                if (formationItems.isEmpty && !_isLoadingUser) {
                  return Column(
                    children: [
                      Text('Aucune formation disponible pour l\'ajout de cours. Vous devez être participant d\'une formation.'),
